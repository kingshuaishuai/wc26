...
<meta name="twitter:card" content="summary_large_image"><meta name="twitter:site" content="@oraclexiai">
...
<div><a href="{rel}index.html">Home</a> · <a href="{rel}blog/index.html">Blog</a> · <a href="{rel}methodology.html">How We Predict</a> · <a href="{rel}about.html">About</a> · <a href="{rel}privacy.html">Privacy</a> · <a href="https://x.com/oraclexiai" rel="me" target="_blank">X @oraclexiai</a> · <a href="mailto:contact@oraclexi.com">Contact</a></div>
</div></footer>
<script>
...
def accuracy_strip():
    ...
    return (f'<a class="acc-strip" href="methodology.html"><span>Model track record</span>'
            f'<span>Outcome called right <b>{META.get("outcome_acc","–")}%</b> '
            f'<i class="vs">vs ~33% guessing</i></span>'
            f'<span class="muted">over {META["played"]} matches · exact score {META.get("score_acc","–")}% (hardest call) · how we predict →</span>'
            f'<span class="upd">Updated {esc(META.get("updated_utc",""))}</span></a>')
...
def build_index():
    ...
    site_ld = jsonld({"@context":"https://schema.org","@graph":[
        ...
        {"@type":"Organization","name":SITE,"url":f"{BASE}/","logo":f"{BASE}/assets/avatar.png",
         "sameAs":["https://x.com/oraclexiai"]}]})
...
METHODOLOGY_HTML = """<p>Every OracleXI prediction is built the same disciplined way — here is exactly how, so you can judge for yourself how much to trust it.</p>
<p><b>1. Real data, not vibes.</b> For each fixture we pull the <b>official group standings and results</b> from football-data.org, both teams' tournament form, and the latest public news on injuries, suspensions and likely line-ups.</p>
<p><b>2. Factor-by-factor reasoning.</b> A language model (DeepSeek) weighs that evidence one factor at a time — qualification stakes, form, missing players, predicted XI, the match-up — and returns a scoreline, win/draw/loss probabilities, a key player and a short analysis citing the factors that drove the call.</p>
<p><b>3. Calibrated to real football.</b> Probabilities are kept honest: draws are common and can be the most likely result, blow-outs are rare, and no single side is ever given more than an 82% chance. We would rather be realistic than loud.</p>
<p><b>4. Updated before kickoff, then frozen.</b> As team news lands we re-run the prediction and log what changed and why on each match page. The moment a match kicks off the call is <b>locked forever</b> — our track record only ever counts the pre-match prediction, never a hindsight edit.</p>
<p><b>5. We publish our misses.</b> After each game we write an honest recap — when we nailed it and when we got it wrong. The accuracy numbers below count every result, good and bad.</p>
<p>OracleXI is an independent project for football fans, not affiliated with FIFA. Predictions are AI-generated for entertainment and information only, and are not betting advice.</p>"""

def _vpill(v):
    m={"exact":("EXACT","var(--lime)"),"hit":("CALLED","var(--lime-dim)"),"miss":("MISSED","var(--coral)")}
    t,c=m.get(v,("—","var(--faint)"))
    return f'<span class="vpill" style="color:{c};border-color:{c}">{t}</span>'

def build_methodology():
    posts=BLOG.get("posts",[])
    n=len(posts)
    hit=sum(1 for p in posts if p.get("verdict") in ("exact","hit"))
    oa=META.get("outcome_acc","–"); sa=META.get("score_acc","–")
    if n:
        rows="".join(f'<tr><td>{esc(p["home"])} v {esc(p["away"])}</td>'
                     f'<td class="pred">{esc(p.get("pred","-"))}</td><td class="act">{esc(p.get("score","-"))}</td>'
                     f'<td>{_vpill(p.get("verdict"))}</td></tr>' for p in posts[:24])
        track=(f'<div class="track"><div class="tnum"><b>{oa}%</b><span>outcome called right<br>'
               f'<i>vs ~33% for a random guess</i></span></div>'
               f'<div class="tnum"><b>{sa}%</b><span>exact scoreline<br><i>the hardest call in football</i></span></div>'
               f'<div class="tnum"><b>{hit}/{n}</b><span>match results called so far</span></div></div>'
               f'<table class="track-tbl"><thead><tr><th>Match</th><th>Predicted</th><th>Actual</th><th>Verdict</th></tr></thead>'
               f'<tbody>{rows}</tbody></table>')
    else:
        track='<p>The live track record will appear here once matches are played.</p>'
    body=METHODOLOGY_HTML+'<div class="sec-head" style="margin-top:30px"><span class="idx">%</span><h2 class="disp">Track Record</h2><div class="line"></div></div>'+track
    out=head("How We Predict & Track Record | "+SITE,
             "How OracleXI's AI World Cup predictions are made — data sources, calibration, and our honest, frozen-at-kickoff accuracy track record.",
             canon="methodology.html")+f"""
<div class="wrap"><div class="crumb"><a href="index.html">Home</a> / How We Predict</div></div>
<section><div class="wrap" style="max-width:820px">
<div class="sec-head"><span class="idx">i</span><h2 class="disp">How We Predict</h2><div class="line"></div></div>
<div class="analysis-box" style="border-left-color:var(--gold)">{body}</div>
</div></section>
""" + foot()
